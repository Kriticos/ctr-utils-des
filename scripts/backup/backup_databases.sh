...
if docker exec "$CONTAINER_MYSQL" \
    ...
    "${PASTA_ZABBIX}/zabbix_backup_${DATA_ATUAL}.sql.gz"; then
...
if [ "$(date +%Y-%m-%d)" = "$DATA_ULTIMO_DIA_MES" ]; then
    ...
    cp "${PASTA_GRAFANA}/grafana_backup_${DATA_ATUAL}.sql.gz" \
       "${PASTA_GRAFANA}/grafana_backup_last_day_of_month_${DATA_ATUAL}.sql.gz"

    cp "${PASTA_ZABBIX}/zabbix_backup_${DATA_ATUAL}.sql.gz" \
       "${PASTA_ZABBIX}/zabbix_backup_last_day_of_month_${DATA_ATUAL}.sql.gz"
